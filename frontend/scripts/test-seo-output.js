...
function schemaTypesOf(html) {
  return [...html.matchAll(/<script\s+type="application\/ld\+json"[^>]*>([\s\S]*?)<\/script>/gi)]
    .flatMap((match) => { try { const schema = JSON.parse(match[1]); return Array.isArray(schema) ? schema.map((item) => item['@type']) : [schema['@type']]; } catch { return []; } })
    .filter(Boolean);
}
...
// Commercial hubs are conversion landing pages: each one the build declares must be a real,
// crawlable, uniquely purposed document with the breadcrumb, schema and enquiry paths intact.
for (const hubPath of report.commercialHubPaths || []) {
  const url = `${siteUrl}${hubPath}`;
  if (!urlSet.has(url)) { fail(url, 'commercial hub missing from sitemap'); continue; }
  const filename = routeFile(url);
  if (!fs.existsSync(filename)) { fail(url, 'commercial hub has no generated document'); continue; }
  const html = fs.readFileSync(filename, 'utf8');
  if (count(html, /<h1(?:\s[^>]*)?>/gi) !== 1) fail(url, 'commercial hub must have exactly one H1');
  if (!/nav\s+aria-label="Breadcrumb"/i.test(html)) fail(url, 'commercial hub missing breadcrumb nav');
  if (!/href="https:\/\/wa\.me\//.test(html)) fail(url, 'commercial hub missing WhatsApp conversion path');
  if (!/href="\/cart"/.test(html)) fail(url, 'commercial hub missing quotation-list path');
  const linkedProducts = new Set([...html.matchAll(/href="(\/[a-z0-9-]+\/[a-z0-9-]+)"/gi)].map((match) => match[1]));
  if (!linkedProducts.size) fail(url, 'commercial hub links no product pages');
  const schemaTypes = schemaTypesOf(html);
  if (!schemaTypes.includes('BreadcrumbList')) fail(url, 'commercial hub missing BreadcrumbList schema');
...
for (const hubPath of report.commercialHubPaths || []) {
  const filename = routeFile(`${siteUrl}${hubPath}`);
  if (!fs.existsSync(filename)) continue;
  const html = fs.readFileSync(filename, 'utf8');
  for (const [kind, value] of [
    ...
    if (hubMeta.has(key)) fail(`${siteUrl}${hubPath}`, `duplicate ${kind} also used by ${hubMeta.get(key)}`);
    else hubMeta.set(key, `${siteUrl}${hubPath}`);
  }
}
