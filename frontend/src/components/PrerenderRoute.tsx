...
export function PrerenderRoute({ data }: { data: RouteData }) {
  ...
  const whatsappHref = product
    ? `https://wa.me/919654102758?text=${encodeURIComponent(`Hi! I'm interested in SKU ${product.sku} - ${product.name}. Please share more details.`)}`
    : 'https://wa.me/919654102758';
  return <div className="min-h-screen bg-background">
    <Header />
    <main className="container mx-auto max-w-7xl px-4 pt-24 pb-16">
      {data.kind === 'product' && product ? <nav aria-label="Breadcrumb" className="mb-6 text-sm text-muted-foreground">
      ...
      </nav> : (data.kind === 'category' || data.kind === 'brand') && <nav aria-label="Breadcrumb" className="mb-6 text-sm text-muted-foreground"><Link to="/">Home</Link><span aria-hidden="true"> / </span><Link to={data.kind === 'brand' ? '/brands' : '/categories'}>{data.kind === 'brand' ? 'Brands' : 'Categories'}</Link><span aria-hidden="true"> / </span><span aria-current="page">{data.heading}</span></nav>}
      {data.kind === 'product' && product ? <article className="grid gap-10 md:grid-cols-2">
        <div className="aspect-square rounded-2xl border bg-card p-6">{product.images?.[0] ? <img src={product.images[0]} alt={product.name} className="mx-auto h-full w-full object-contain" /> : <div className="h-full w-full" />}</div>
